...
func (c *Cipher) Init() error {
	if c.Key == "" {
		return errors.New("Cipher.Key can't be empty")
	}
	if c.Storage == nil {
		return errors.New("Cipher.Storage can't be nil")
	}
...
func (c Cipher) Get(ctx context.Context, key string) (io.ReadCloser, error) {
	span, ctx := ddTracer.StartSpanFromContext(ctx, "Cipher.Get")
	defer span.Finish()
...
func (c Cipher) Put(ctx context.Context, key string, reader io.Reader) error {
	span, ctx := ddTracer.StartSpanFromContext(ctx, "Cipher.Put")
	defer span.Finish()
	...
	if _, err := io.ReadFull(rand.Reader, nonce); err != nil {
		return fmt.Errorf("fail to initialize nonce: %w", err)
	}
	result := c.client.Seal(nonce, nonce, payload, nil)
	return c.Storage.Put(ctx, key, bytes.NewBuffer(result))
}
