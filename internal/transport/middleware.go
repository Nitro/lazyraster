...
func (m middleware) logger(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasPrefix(r.RequestURI, "/documents") && r.URL.Query().Get("token") != "" {
			endpoint, err := url.Parse(requestURI)
			if err != nil {
				m.writer.error(r.Context(), w, "Could not parse Request.RequestURI", nil, http.StatusInternalServerError)
				return
			}
			qs := endpoint.Query()
			qs.Set("token", "REDACTED")
			endpoint.RawQuery = qs.Encode()
			requestURI = endpoint.String()
		}
